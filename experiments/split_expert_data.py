...
def convert_model_to_npz(model_path, output_path="best_model.npz"):
    ...
    # 1. Unpack the zip archive into a temporary folder
    ...
    # 2. Get the structure (shapes and dtypes) of the checkpoint without loading the actual data
    structure = checkpointer.structure(tmp_dir)

    # 3. Create a "dummy target" of pure NumPy zeros matching the checkpoint's shape.
    ...
    # 4. Generate restore_args from our un-sharded dummy target
    restore_args = orbax_utils.restore_args_from_target(dummy_target)

    print("Restoring checkpoint weights to CPU...")
    # 5. Restore the actual checkpoint data safely using the dummy blueprint
    ...
    # 6. Flatten the nested dictionaries and ensure they are standard NumPy arrays
    ...
    # 8. Clean up the temporary extracted folder
    shutil.rmtree(tmp_dir)
