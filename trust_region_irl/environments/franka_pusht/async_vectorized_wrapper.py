...
class AsyncVectorEnvWithSkipping(gym.vector.AsyncVectorEnv):
    """
    A subclass of gym.vector.AsyncVectorEnv to run parallel environments while training
    """
    ...
    def step_async(self, actions):
        """
        Sends an action to each worker in the pool.
        """
        self._assert_is_running()
        # check if the worker can receive new actions
        ...
        # Pass the actions to each pipe
        for pipe, action, skipped in zip(self.parent_pipes, actions, self.skipped_envs):
            if not skipped:
                pipe.send(("step", action))

        # Set the stat to WAITING_STEP till it finishes
        self._state = AsyncState.WAITING_STEP

    
    def step_wait(self, timeout=None):
        """
        Collect reward, terminated and truncated observations from each worker.
        """
        self._assert_is_running()
