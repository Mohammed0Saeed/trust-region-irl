...
class RLXInfo(gym.Wrapper):
    ...
    def feature_from_transition(self, state, action):
        # feature_from_transition is a stateless staticmethod, so call it on the
        # class directly. This avoids reaching into sub-env instances, which do
        # not exist on AsyncVectorEnv (workers live in separate processes).
        return FrankaPushT.feature_from_transition(state, action)
