"""PushT FR3 environment (MJX) for TRIRL, recalibrated to the real lab rig.

Same task/scene as pusht_mjx (the pusher/EE tool geometry already matches the
real custom_pusher_ee exactly), but with control constants pulled from the
calibrated real setup (franka_ros2_ws/src/franka_pusht/config/pusht_robot.yaml
and pusht_common.yaml, finalized 2026-08-18): real max linear speed, real arm
home pose, and a real TCP-relative workspace envelope enforced as an episode
divergence bound. Block/goal placement is left unchanged from pusht_mjx (the
real setup only tracks the robot mount live, via an OptiTrack marker, so there
is no static baked number to substitute there).
...
from trust_region_irl.environments.real_pusht_mjx.state import State
from trust_region_irl.environments.real_pusht_mjx.box_space import BoxSpace
from trust_region_irl.environments.real_pusht_mjx.viewer import MujocoViewer


ARM_JOINT_NAMES = [f"fr3_joint{i}" for i in range(1, 8)]
# Real robot's null-space home pose (pusht_common.yaml: home_joint_positions).
QHOME = np.array([0.51199203, 0.1014329, 0.0, -2.356, 0.0, 1.571, 0.785])
...
class PushT:
    ...
    MAX_SPEED = 0.08  # real robot's calibrated max_linear_speed (pusht_robot.yaml)
    CONTROL_FREQ = 20
    KP_ORI = 10.0
    Z_HOLD = 0.045  # ee height the IK servos toward
    # Real safety envelope: TCP position relative to goal (pusht_robot.yaml
    # workspace_x/y/z). Enforced below as an episode-divergence bound, same
    # role the real safety_node plays by refusing motion outside these bounds.
    ...
    def _step(self, state, action):
        ...
        # real-workspace guard: TCP left the calibrated safe envelope
        # (pusht_robot.yaml workspace_x/y/z) -- mirrors the real safety_node
        # refusing motion outside these bounds.
        ee_pos_rel = jnp.nan_to_num(data.xpos[self.ee_body_id] - self.goal_pos, nan=jnp.inf)
        outside_workspace = (
            (ee_pos_rel[0] < self.WORKSPACE_X[0]) | (ee_pos_rel[0] > self.WORKSPACE_X[1])
            | (ee_pos_rel[1] < self.WORKSPACE_Y[0]) | (ee_pos_rel[1] > self.WORKSPACE_Y[1])
            | (ee_pos_rel[2] < self.WORKSPACE_Z[0]) | (ee_pos_rel[2] > self.WORKSPACE_Z[1])
        )
        diverged = (jnp.nan_to_num(jnp.linalg.norm(block_pos_raw), nan=jnp.inf) > self.DIVERGE_BOUND) \
            | outside_workspace | jnp.any(jnp.isnan(data.qpos)) | jnp.any(jnp.isnan(data.qvel))
    ...
    def feature_from_transition(self, observation, action, eps=1e-4):
        ...
        elif self.feature_fn == "base_without_ctrl":
